...
class SimpleDemoStateMachine(object):
    ...
    def searching_position_control(self, x, v, contact):
        ...
        p_des = self.target_pos_estimate - np.array([0.2, 0.2, 0.0]) - np.array([0, 0, 0.25])
        yaw_des = self.target_yaw_estimate
    ...
    def control(self, x, v, contact):
        ...
        if self.state == State.TAKEOFF:
            ...
            if np.linalg.norm(x[:3] - self.reference_pos) < 0.05 and np.linalg.norm(v[:3]) < 0.01:
            ...
            self.reference_pos = ctrl["p_des"]  
            if ((np.linalg.norm(x[:3] - self.reference_pos) < 0.05) and
               (self.alpha < 0.05).all()):
